...
<style>
:root{
  --brass-dark:#594622; --brass:#BF9A54; --rust:#402A10; --paper:#F2F2F2;
  --line: rgba(191,154,84,.22); --glass: rgba(242,242,242,.07); --shadow: rgba(0,0,0,.40);
  --mono: ui-monospace, Menlo, Consolas, monospace; --sans: system-ui, Segoe UI, Roboto, Arial;
...
.nav{display:flex; justify-content:space-between; align-items:center; gap:10px; padding:14px 0;}
.brand h1{margin:0; font-size:14px; text-transform:uppercase; letter-spacing:.7px;}
.brand p{margin:0; font-family:var(--mono); font-size:11px; color:rgba(242,242,242,.68);}
a.link{
  font-family:var(--mono); font-size:12px; text-decoration:none; color:rgba(242,242,242,.85);
  border:1px solid rgba(191,154,84,.20); background: rgba(242,242,242,.06);
  padding:9px 10px; border-radius:999px;
}
a.link:hover{color:var(--brass); border-color: rgba(191,154,84,.35);}
...
.chatTop strong{font-size:13px; letter-spacing:.8px; text-transform:uppercase;}
.status{
...
.inputBar button:active{ transform: scale(.98); }
...
.sideTop strong{display:block; font-size:13px; letter-spacing:.8px; text-transform:uppercase;}
.sideTop span{display:block; font-family:var(--mono); font-size:11px; color:rgba(242,242,242,.68); margin-top:4px; line-height:1.4;}
...
.box h3{margin:0 0 8px; font-size:12px; text-transform:uppercase; letter-spacing:.8px;}
.box p{margin:0; font-family:var(--mono); font-size:11.5px; color:rgba(242,242,242,.74); line-height:1.55; white-space:pre-line;}
.iceRow{display:flex; gap:10px; flex-wrap:wrap; align-items:center;}
.btnGhost{
...
.btnGhost:hover{ border-color: rgba(191,154,84,.40); color: var(--brass); }
.small{
  font-family:var(--mono);
  font-size:11px;
  color:rgba(242,242,242,.70);
...
<main>
  ...
  <section class="panel">
    <div class="chatTop">
      <strong id="title">Secure Channel</strong>
...
(() => {
  ...
  const ice1 = $("#ice1"), ice2 = $("#ice2"), ice3 = $("#ice3");
...
RISK: ${match.risk}/100`;
  ...
  ice1.addEventListener("click", ()=>{ input.value = openers[0]; input.focus(); pop("Inserted opener."); });
  ice2.addEventListener("click", ()=>{ input.value = openers[1]; input.focus(); pop("Inserted opener."); });
  ice3.addEventListener("click", ()=>{ input.value = openers[2]; input.focus(); pop("Inserted opener."); });
  ...
  // chat storage
  function getChats(){ return load(STORE.chats, {}); }
  function setChats(v){ save(STORE.chats, v); }
